...
namespace FacturaApp.Tests
{
    public abstract class FacturaAppTestBase : AbpIntegratedTestBase<FacturaAppTestModule>
    {
        ...
        protected void UsingDbContext(Action<FacturaAppDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FacturaAppDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FacturaAppDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FacturaAppDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FacturaAppDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FacturaAppDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FacturaAppDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FacturaAppDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FacturaAppDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FacturaAppDbContext, Task<T>> func)
        {
            T result;
